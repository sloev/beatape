...
class DBNBeatTrackingProcessor(OnlineProcessor):
    ...
    def process_online(self, activations, reset=True, **kwargs):
        ...
        # visualisation stuff 
        # MKE: modifications to compute strength even if not visualize
        ...
        # return beat(s) and strength  (MKE: originally only returned beats array)
        q = kwargs.get('q')
        if len(np.array(beats_)) > 0:
            t = ('tempo', self.tempo)
            if q is not None:
                q.put(t)
        ...
        if q is not None:
            q.put(s)
        return (np.array(beats_), self.tempo, self.beat_counter, self.strength)
